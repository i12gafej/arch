...
function splitBoxes(boxes) {
  return {
    ...
    misc: boxes.filter(
      (box) =>
        !SUBMODULE_KINDS.has(box.kind) &&
        !DOMAIN_KINDS.has(box.kind) &&
        !APP_KINDS.has(box.kind) &&
        !INFRA_KINDS.has(box.kind) &&
...
function moduleLayout(context, boxes, helpers) {
  ...
  const minBottom = Math.max(
    coreTop + coreHeight,
    infraRect.y + infraRect.h,
    portRect.y + portRect.h
  );
...
function submoduleLayout(context, boxes, helpers) {
  const content = context.contentRect;
  const gap = helpers.gap;
  const portWidth = PORT_DOCK_WIDTH;
  const infraWidth = INFRA_WIDTH;
  ...
  const hasDomain = domainItems.length > 0;
  const hasApp = sections.app.length > 0;
  const minCoreWidth = hasDomain && hasApp ? 220 * 2 + gap : 220;
  const coreWidth = Math.max(240, minCoreWidth, content.w - portWidth - infraWidth - gap * 2);
  ...
  if (hasDomain && hasApp) {
    let domainWidth = Math.max(220, Math.floor(coreRect.w * 0.5));
    let appWidth = coreRect.w - domainWidth - gap;
    if (appWidth < 220) {
      appWidth = 220;
      domainWidth = coreRect.w - appWidth - gap;
    }
    domainRect = { x: coreRect.x, y: coreRect.y, w: domainWidth, h: coreRect.h };
    appRect = { x: domainRect.x + domainRect.w + gap, y: coreRect.y, w: appWidth, h: coreRect.h };
  } else if (hasDomain) {
    domainRect = { x: coreRect.x, y: coreRect.y, w: coreRect.w, h: coreRect.h };
  } else if (hasApp) {
    appRect = { x: coreRect.x, y: coreRect.y, w: coreRect.w, h: coreRect.h };
  }
  ...
  const layoutRightEdge = portRect.x + portRect.w;
  const minBottom = Math.max(coreRect.y + coreRect.h, infraRect.y + infraRect.h, portRect.y + portRect.h);
...
export function layoutContext(context, boxes, helpers) {
  ...
  if (context.kind === "submodule") {
    return submoduleLayout(context, boxes, helpers);
  }
